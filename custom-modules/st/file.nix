...
/* c */ ''
   ...
   */
  static char *shell = "${toString cfg.shell}";
  char *utmp = NULL;
   ...
   * 7: Snowman ("☃")
   */
  static unsigned int cursorshape = ${toString cfg.cursorShape};
   ...
   */
  static KeySym mappedkeys[] = { ${concatStringsSep ", " cfg.mappedKeys} };

  /*
   * State bits to ignore when matching key or button events.  By default,
   * numlock (Mod2Mask) and keyboard layout (XK_SWITCH_MOD) are ignored.
   */
  static uint ignoremod = (${cfg.modifier.ignoreMod});
  ...
  static Key key[] = {
    ...
    ${concatMapStringsSep ",\n  " (
      value:
      ''{ ${toString value.keysym}, ${toString value.mask}, "${value.string}", ${toString value.appkey}, ${toString value.appcursor} }''
    ) cfg.key.keys}
    ${
      if cfg.key.useDefault then
        ''
